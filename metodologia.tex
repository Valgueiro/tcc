...
%\url{http://bit.ly/ArtigoQuestoesEntrevista
...
O roteiro da entrevista é dividida em 5 seções: 
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{detalhamento_dos_entrevistados.png}
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[scale=0.40]{metodologia_tcc.png}
\end{center}
...
A Figura \ref{trecho_tabela_t1} ilustra um trecho dos agrupamentos realizados. Por exemplo, o códigos [17] do entrevistado P1 e o código [14] do entrevistado P2, foram agrupados à prática HC (\emph{Health Checks}). Similarmente, os códigos [9] de P1 e [21] de P2 foram agrupados à prática DOC (\emph{Developer on Call}). Este processo foi então replicado para cada uma das 11 entrevistas, gerando uma versão estendida da planilha mostrada na Figura \ref{trecho_tabela_t1} com um total de 11 linhas e 10 colunas.

Ao final do processo ainda haviam 3 entrevistas que não geraram nenhum código relacionado a prática de \emph{Dark Launch}. Para estes casos, foi enviado um email diretamente para cada um dos entrevistados com a definição do estudo original da prática e foi perguntado se o entrevistado utilizava ou não a mesma, o que gerou mais 3 códigos. Ao final, 292 códigos surgiram ao todo; A lista de todos os códigos está disponível em (link será adicionado após revisão).

%\url{http://bit.ly/ArtigoCodigos}.

Após a etapa de geração e revisão dos códigos, estes foram então agrupados, onde aplicável, na prática específica com a qual aquele código se relacionava. Este agrupamento gerou a Tabela Entrevistado-Pratica-Codigos,\footnote{A Tabela Entrevistado-Pratica-Codigos está disponível em (link será adicionado após revisão)} que mostra uma visão geral de cada relação entre prática e entrevistado, contendo a nota dada e os códigos que justificam a nota. A nota é demonstrada pela cor presente em cada célula: branco significa não utiliza, verde claro mostra que o participante utiliza parcialmente, e verde mais escuro, totalmente. A tabela contém 147 dos 292 códigos gerados. A Figura \ref{trecho_tabela_t1} apresenta um trecho da Tabela Entrevistado-Pratica-Codigos para os entrevistados P1 e P2 e os códigos de cada um relacionados às práticas HC e DOC.

%\url{http://bit.ly/ArtigoTabelaEPC}
...
\begin{figure}[ht]
    ...
    \caption[Trecho da Tabela Entrevistado-Pratica-Codigos]{
        Trecho da Tabela Entrevistado-Pratica-Codigos, com os códigos derivados das entrevistas agrupados por prática. As cores de cada célula representam a nota dada para determinada prática, de acordo com sua adoção pelo participante. 0: Branco (prática não é utilizada), 1: Verde Claro (utilizada parcialmente) e 2: Verde Escuro (utilizada totalmente).

}\label{trecho_tabela_t1}
\end{figure}

Para identificar padrões e abstrações nos códigos agrupados na Tabela Entrevistado-Pratica-Codigos, foi feito um trabalho de agrupamento semântico, gerando por fim 17 novas super categorias\footnote{A tabela com todas as super categorias geradas pode ser encontrada em (link será adicionado após revisão)} através do processo de \emph{Thematic Analysis} \cite{groundedTheory}. Esse processo é ilustrado pela etapa de ``Criação de super categorias'' na Figura \ref{fluxograma_metodologia}. As super categorias tinham como objetivo agrupar códigos semelhantes ou de mesma semântica para facilitar na reflexão dos dados. Um exemplo de super categoria relacionada à prática de \emph{Trunk Based Development} [TBD] pode ser encontrado na Figura \ref{exemplo_super_categoria}.

%\url{http://bit.ly/ArtigoSupCategorias}

\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.8\linewidth]{exemplo_super_categoria.png}
\end{center}
...
%\url{http://bit.ly/ArtigoDescobertasAdicionais}